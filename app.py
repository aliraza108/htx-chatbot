...
# =====================================================
# API Key setup (⚠️ In production, put this in Vercel env)
# =====================================================
part1 = "sk-proj"
...
async def debug():
    ...
    try:
        result = await Runner.run(Triage_Agent, input="Say the name of your planet in one word.")
        report["runner_test"] = {"ok": True, "final_output": str(result.final_output)}
    except Exception as e:
        import traceback
        report["runner_test"] = {"ok": False, "error": str(e), "trace": traceback.format_exc()}
    return report


@app.post("/chat")
async def chat_endpoint(req: Request):
    """Chat endpoint using Triage_Agent"""
    body = await req.json()
    ...
    try:
        result = await Runner.run(Triage_Agent, input=msg)
